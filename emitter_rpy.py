...
def emit_program(el):
	return readfile('emitter_rpy_header.py'), emit(el), readfile('emitter_rpy_footer.py')
...
def emit(el):
	if isinstance(el, ast.Function):
		...
		for x in el.value:
			ret += indent(emit(x))
			ret += '\n'
		ret += 'state.stack.push(StackElement(val_fn=fn))'
		return ret
	if isinstance(el, ast.Number):
		return f'state.stack.push(StackElement(val_int={el.value}))'
